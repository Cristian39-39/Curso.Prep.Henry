...
function incrementarPorUno(array) {
  ...
  // y devuelve el array
  // Tu código:
  var nuevoarray = array.map(function(cb){
    return cb+1;
  });
  return nuevoarray;
}
...
function agregarNumeros(array) {
  ...
  for(let i = 0; i< array.length; i++) {
    if(array[i]%1===0) sumatoria += array[i];
  }
  return sumatoria;
}
...
function numeroMasGrande(numeros) {
  ...
  for(let i = 0; i < numeros.length; i++) {
    if(numeros[i] > salida) salida = numeros[i];
  }
  return salida;
}
...
function mesesDelAño(array) {
  ...
  if(salida.includes('Enero')) {
    if(salida.includes('Marzo')) {
      if(salida.includes('Noviembre')) return salida;
    }
  }
...
function breakStatement(numero) {
  ...
  do {
    ...
    tem += 2;
    salida.push(tem);
    if(tem === i) return 'Se interrumpió la ejecución'
  } while (i < 10);
  return salida;
}
